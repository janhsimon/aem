...
{
  igSetNextWindowPos((ImVec2_c){ screen_width - screen_width / 4, (screen_height / 4) * window_index }, ImGuiCond_Once,
                     (ImVec2_c){ 0.0f, 0.0f });

  igSetNextWindowSizeConstraints((ImVec2_c){ screen_width / 4, screen_height / 4 },
                                 (ImVec2_c){ screen_width, screen_height }, NULL, NULL);
...
void add_texture_window_image(unsigned int texture_index, uint32_t screen_width, uint32_t screen_height)
{
  ...
  igImage(ref, (ImVec2_c){ draw_width, draw_height }, (ImVec2_c){ 0.0f, 1.0f }, (ImVec2_c){ 1.0f, 0.0f });
}
