...
void load_view_weapon(const struct AEMModel* model_)
{
  model = model_;
...
void update_view_weapon(bool moving, float delta_time)
{
  if (is_shooting)
  {
    if (shoot_channel->time >= aem_get_model_animation_duration(model, 2) - 0.1f)
    {
      ...
      aem_blend_to_animation_mixer_channel(mixer, 0); // To idle
    }
  }
  else
  {
...
void get_view_weapon_world_matrix(float world_matrix[16])
{
...
unsigned int get_view_weapon_joint_transform_texture()
{
  return joint_transform_texture;
}